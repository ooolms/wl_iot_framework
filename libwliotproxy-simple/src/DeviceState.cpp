...
DeviceState::DeviceState(StreamWriter *wr)
{
	mCommands=0;
	mAddParams=0;
...
void DeviceState::prepareCommands(unsigned int count)
{
	if(mCommands)return;
...
void DeviceState::prepareCommand(unsigned int index,const char *commandName,unsigned int paramsCount)
{
	if(index>=mCommandsCount)return;
	mCommands[index].prepare(commandName,paramsCount);
}

void DeviceState::prepareAdditionalParameters(unsigned int count)
{
...
void DeviceState::prepareAdditionalParameter(unsigned int index,const char *name)
{
	if(index>=mAddParamsCount)return;
	mAddParams[index].prepare(name);
}

void DeviceState::setCommandParamState(unsigned int commandIndex,unsigned int paramIndex,const char *value)
{
...
void DeviceState::setAdditionalParamState(unsigned int index,const char *value)
{
	if(index>=mAddParamsCount)return;
...
void DeviceState::notifyCommandParamChanged(unsigned int commandIndex, unsigned int paramIndex)
{
...
void DeviceState::writeCommandParamState(unsigned int commandIndex,unsigned int paramIndex)
{
	writer->writeArg(mCommands[commandIndex].command,strlen(mCommands[commandIndex].command));
	writeUInt(paramIndex);
	char *v=mCommands[commandIndex].paramsValues[paramIndex];
	if(v)writer->writeArg(v,strlen(v));
	else writer->writeArgNoEscape("");
}

void DeviceState::notifyAdditionalParamChanged(unsigned int index)
{
	if(!writer->beginWriteMsg())return;
	writer->writeArgNoEscape("statechanged");
...
void DeviceState::writeAdditionalParamState(unsigned int index)
{
...
DeviceState::CommandState::CommandState()
{
	command=0;
	paramsCount=0;
	paramsValues=0;
...
void DeviceState::CommandState::prepare(const char *cmd,unsigned int pCount)
{
	if(command)return;
...
void DeviceState::CommandState::updateValue(unsigned int index,const char *val)
{
...
void DeviceState::AdditionalParamState::updateValue(const char *val)
{
	if(paramValue)
		delete[] paramValue;
