...
void IotServerDevice::processMessage(const ARpcMessage &m)
{
	...
	if(m.title==ARpcConfig::identifyMsg)
		streamParser.emitNewMessage(ARpcMessage(ARpcConfig::deviceInfoMsg,
			QByteArrayList()<<devId.toByteArray()<<devName));
	else if(m.title==ARpcConfig::funcCallMsg)
	{
		if(m.args.count()<2)
		{
			streamParser.emitNewMessage(ARpcMessage(ARpcConfig::funcAnswerErrMsg,
				QByteArrayList()<<"invalid arguments"));
			return;
		}
		QByteArray callId=m.args[0];
		QByteArrayList retVal;
		if(commands->devices()->execDeviceCommand(devId.toByteArray(),m.args[1],m.args.mid(2),retVal))
			streamParser.emitNewMessage(ARpcMessage(ARpcConfig::funcAnswerOkMsg,QByteArrayList()<<callId<<retVal));
		else streamParser.emitNewMessage(ARpcMessage(ARpcConfig::funcAnswerErrMsg,QByteArrayList()<<callId<<retVal));
	}
	else if(m.title==ARpcConfig::devSyncMsg)
		streamParser.emitNewMessage(ARpcMessage(ARpcConfig::devSyncrMsg));
}
