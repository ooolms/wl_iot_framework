...
void ARpcFSSensorStorageHelper::writeAttribute(const QByteArray &str,const QVariant &var)
{
	if(mDbPath.isEmpty())
		return;
	mSettings->beginGroup("attributes");
	mSettings->setValue(QString::fromUtf8(str),var);
	mSettings->endGroup();
	mSettings->sync();
}

QVariant ARpcFSSensorStorageHelper::readAttribute(const QByteArray &str)
{
	if(mDbPath.isEmpty())
		return QVariant();
	mSettings->beginGroup("attributes");
	QVariant v=mSettings->value(QString::fromUtf8(str));
	mSettings->endGroup();
	return v;
}
