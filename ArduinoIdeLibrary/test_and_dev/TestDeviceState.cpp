...
void testStateWriteCb(const char *str,unsigned long sz)
{
	buffer.append(QByteArray(str,sz));
}

void testStateCommandCb(const char *cmd,const char *args[],unsigned char argsCount,ARpc *parser)
{
...
bool testDeviceState()
{
	...
	*/
	ARpc parser(3000,&testStateCommandCb,&testStateWriteCb,devId,devName);
