...
VdevMeasbCommand::VdevMeasbCommand(QtIODeviceWrap *d,IotProxyCommandProcessor *p)
	:ICommand(d,p)
{
}
...
bool VdevMeasbCommand::processCommand(CallContext &ctx)
{
	...
	VirtualDevice *vDev=IotProxyInstance::inst().devices()->virtualDeviceByIdOrName(devIdOrName);
	if(!vDev)
	{
