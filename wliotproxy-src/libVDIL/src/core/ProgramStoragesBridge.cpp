...
ProgramStoragesBridge::ProgramStoragesBridge(WLIOTVDIL::Program *p)
{
	prg=p;
}

void ProgramStoragesBridge::prepareToStart()
{
	for(BaseBlock *b:prg->allBlocks())
	{
		...
		if(b->blockName()==StorageSourceBlock::mBlockName)
		{
			...
			mConnMap[id]=connect(st,&ISensorStorage::newValueWritten,
				this,&ProgramStoragesBridge::onNewValue);
		}
	}
}

void ProgramStoragesBridge::cleanupAfterStop()
{
...
void ProgramStoragesBridge::onNewValue()
{
