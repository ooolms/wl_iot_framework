...
QScriptValue JSSessionsStorage::listSessions()
{
	...
	if(!reinterpret_cast<ISessionSensorStorage*>(stor)->listSessions(ids,titles))
		return js->nullValue();
...
bool JSSessionsStorage::openSession(QScriptValue sessionId)
{
	...
	return (reinterpret_cast<ISessionSensorStorage*>(stor))->openSession(id);
}
...
bool JSSessionsStorage::closeSession(QScriptValue sessionId)
{
	QUuid id(sessionId.toString());
	if(id.isNull())return false;
	return (reinterpret_cast<ISessionSensorStorage*>(stor))->closeSession(id);
}
...
QScriptValue JSSessionsStorage::getSessionAttribute(QScriptValue sessionId,QScriptValue key)
{
	...
	QByteArray var;
	if(!reinterpret_cast<ISessionSensorStorage*>(stor)->getSessionAttribute(
...
quint64 JSSessionsStorage::valuesCount(QScriptValue sessionId)
{
	...
	return reinterpret_cast<ISessionSensorStorage*>(stor)->valuesCount(id);
}
...
quint64 JSSessionsStorage::findInGTIndex(QScriptValue sessionId,QScriptValue ts)
{
	...
	if(id.isNull())
		return 0;
	return reinterpret_cast<ISessionSensorStorage*>(stor)->findInGTIndex(id,(quint64)ts.toNumber());
}
...
QScriptValue JSSessionsStorage::valueAt(QScriptValue sessionId,QScriptValue index)
{
	...
	if(id.isNull())
		return js->nullValue();
	SensorValue *val=reinterpret_cast<ISessionSensorStorage*>(stor)->valueAt(id,(quint64)index.toNumber());
...
bool JSSessionsStorage::isSessionOpened(QScriptValue sessionId)const
{
	QUuid id(sessionId.toString());
	if(id.isNull())return false;
	return reinterpret_cast<ISessionSensorStorage*>(stor)->isSessionOpened(id);
}
...
bool JSSessionsStorage::setMainReadSessionId(QScriptValue sessionId)
{
	QUuid id(sessionId.toString());
	if(id.isNull())return false;
	return reinterpret_cast<ISessionSensorStorage*>(stor)->setMainReadSessionId(id);
}
